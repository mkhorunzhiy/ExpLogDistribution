...
class DistributionController < ApplicationController
  def index
    ...
    if generation_count && max_x && p1 && a2 
    ...
    if generation_count < 1 || max_x <= 0 || p1 <= 0 || a2 <= 0 
      return
    end
    ...
    pdf_calculation_lambda = -> (x) { ProbabilityDensityFunction.solve(p1, a2, x) }
    pdf_mode_value = ProbabilityDensityFunction.mode(p1, a2)
    pdf_mean_value = ProbabilityDensityFunction.mean(p1, a2)
    pdf_variance_value = ProbabilityDensityFunction.variance(p1, a2)
    pdf_deviation_value = ProbabilityDensityFunction.deviation(p1, a2, total_generations_count)
    pdf_maximum_value = ProbabilityDensityFunction.maximum_value(p1, a2)
